...
Два входа в одну и ту же модель — контейнер `stt`. Второй модели в стеке нет и
заводить её не надо: она стоит полтора гигабайта памяти и даёт два разных
ответа на одну запись.

| Метод | Путь | Кто зовёт |
|---|---|---|
| `POST` | `/api/transcriptions/` | браузер через QMS: сразу `task_id`, результат приходит callback-ом с подписью HMAC-SHA256 |
| `POST` | `/api/transcriptions/sync` | телефон через backend PushToTalk: текст возвращается на том же соединении |
| `GET` | `/api/transcriptions/capabilities` | форматы, лимиты, коды ошибок |

Асинхронный вариант существует потому, что запись приходит со страницы, которая
не должна замирать. Телефону callback принимать некуда — он и так держит кнопку
и ждёт, поэтому получает текст ответом.

### `POST /api/transcriptions/sync`

`multipart/form-data`: `audio_file` плюс необязательный `language`.

```json
{"status": "ok", "text": "партия B-154 закончила замес",
 "transcript": "партия B-154 закончила замес",
 "language": "ru", "duration_ms": 1700, "took_ms": 2840}
```

`text` и `transcript` — одно и то же поле под двумя именами: асинхронный
контракт назывался `transcript`, телефон читает `text`.

Ошибки разделены по тому, имеет ли смысл повтор:

| Код | HTTP | Значит |
|---|---|---|
| `speech_unavailable` | 503 | контейнер распознавания не отвечает или в образе нет ffmpeg — повторить позже |
| `unsupported_format` | 415 | этот формат не разобрать — повтор не поможет |
| `validation_error` | 422 | пустая запись, тишина, таймаут декодирования |
