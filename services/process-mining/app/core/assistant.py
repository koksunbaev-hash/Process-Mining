"""Ассистент: вопрос по-русски - ответ по журналу событий.

Устроено как разговор с инструментами. Модель не видит журнала и не умеет
его читать; она умеет спросить: «покажи узкие места», «что было с делом
1042», «сколько раз повторялась формовка». Каждый такой вопрос - функция
ниже, обычный запрос к таблице. Модель получает ответ и складывает из
нескольких ответов один человеческий.

Почему так, а не «отдать журнал целиком»: журнал не влезет, а если резать
его на куски, модель начнёт считать сама - и ошибётся. Пусть считает
pandas, а модель объясняет.
...
SYSTEM = (
    "Ты помощник по анализу производственного процесса. У тебя есть журнал "
    "событий и функции, которыми его можно расспрашивать.\n"
    "Правила:\n"
    "1. Прежде чем отвечать про числа - возьми их функцией. Не помнишь, не "
    "видел, не уверен - спроси данные, а не выдумывай.\n"
    "2. Если данных для ответа нет, так и скажи: «в журнале этого не видно». "
    "Это нормальный ответ, лучше выдуманного.\n"
    "3. Отвечай по-русски, коротко и по делу: два-четыре предложения, если "
    "вопрос не требует большего. Без разметки и списков-звёздочек.\n"
    "4. Длительности пиши так, как их вернула функция.\n"
    "5. Ты видишь только этот журнал. Про заказы, цены и людей вне журнала "
    "отвечай, что таких данных у тебя нет."
)
...
def ask(
    ...
    for _ in range(MAX_STEPS):
        ...
        message = llm.chat(settings, messages, tools=TOOL_SPECS, thinking=False)
        if message is None:
            return {
                "available": False,
