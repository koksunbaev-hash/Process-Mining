...
def board_day_start(
    ...
    """Начало производственного дня, который доска показывает сейчас.

    Это последние наступившие 8:00 - момент, с которого колонка «Готово»
    считается чистой. Смысл именно в «последних наступивших», а не в «сегодня
    в 8:00»: ночная смена печёт с восьми вечера до восьми утра, и в три часа
    ночи её выпуск обязан оставаться на доске. Взяли бы сегодняшние 8:00 -
    он пропадал бы прямо из-под рук у смены, которая его и сделала.

    В восемь утра граница перешагивает вперёд, и всё, что сделано до неё,
    с доски уходит - включая ночную смену. Так и задумано: доска это рабочий
    стол, а не отчёт. Сделанное посчитано в листе заказа и лежит на складе.
    """
    boundary = moment.replace(
        hour=shift_one_start.hour,
        minute=shift_one_start.minute,
        second=0,
        microsecond=0,
    )
    if moment < boundary:
